...
Now that we have exposed a [Privacy Portal](https://github.com/blindnet-io/devkit-quickstart/blob/main/tutorials/intro/web/expose-request-interface.html)
...
We need a server that generates tokens for the Privacy Portal to use in its requests to the Privacy
Computation Engine.

We provide two bare-bones implementations, one in [Java](https://github.com/blindnet-io/devkit-quickstart/tree/main/tutorials/intro/server/java)
...
Clone the [devkit-quickstart](https://github.com/blindnet-io/devkit-quickstart) repository using Git and copy
(or `cd` into) the `/tutorials/intro/server/java` directory. Then, use Maven to compile and run the server.
...
This function gets a [user token](/docs/authentication/tokens) for the email entered, then sets the `api-token` attribute of the Privacy Portal.

:::tip Checkpoint

Now we can build a privacy request in the Privacy Portal and send it to the PCE for processing. Great!

The HTML file with our work so far can be found [here](https://github.com/blindnet-io/devkit-quickstart/blob/main/tutorials/intro/web/add-authentication.html).
