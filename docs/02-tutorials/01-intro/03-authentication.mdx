...
In [part one](./prci-set-up), we captured the created request through an event emitted by the PRCI. Now, let's direct requests to the privacy computation engine from [part two](./pce-set-up) instead.

To do this, add an attribute indicating the URL of the machine hosting the PCE:
...
The HTML file with our work so far can be found [here](https://github.com/blindnet-io/devkit-quickstart/blob/main/web/add-authentication.html).

:::

Next, let's see an example of placing a request.