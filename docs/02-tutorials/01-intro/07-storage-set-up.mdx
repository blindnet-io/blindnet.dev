...
For that, we use the `Builder.setSubjectMapper()` method, which only need a function returning the desired object based on a given selector (here, the user's email).
