...
It is possible to define serval records for one Data Category, e.g.

| Data in database | Privacy Scope | Legal bases |
| :----: | :-----: | :--------|
| `USER`, `e_mail_address` | `CONTACT.EMAIL` x {`COLLECTION`, `STORING`, `USING`} x {`SERVICES`,`COMPLIANCE`} | `CONTRACT` |
| `USER`, `e_mail_address` | `CONTACT.EMAIL` x `USING` x {`MARKETING`,`ADVERTISING`} | `LEGITIMATE-INTEREST`|
| `USER`, `e_mail_address` | `CONTACT.EMAIL` x `SHARING` x {`ADVERTISING`} | `CONSENT`|
