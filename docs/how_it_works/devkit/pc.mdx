...
- when a particular `data-reference` is given:
    ...
    - Removes all the Triples that are left with no active Legal Base ([intersection](#intersection) operation)
...
Finally, it removes all the Triples that are left with no active Legal Base ([intersection](#intersection) operation) as a consequence of consent removal.
