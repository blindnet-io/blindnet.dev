...
1. A list of user ids (specified in a [temporary token](/docs/references/glossary#temp-user-token)) is sent to blindnet to obtain the corresponding [public keys](/docs/references/glossary#public-key)
1. An [ephemeral symmetric key](/docs/references/glossary#data-key) is generated
...
Encryption is completed by a user of your application ([registered](/docs/references/glossary#registered-user) or [unregistered](/docs/references/glossary#anonymous-user)) to another registered user. A user does not need to be logged in to blindnet to encrypt the data.
Data can be encrypted for a single user, a list of users or a [user group](/docs/references/glossary#user-groups).
...
To encrypt the data for a specific user, obtain a [temporary token](/docs/references/glossary#temp-user-token) using the following **Server SDK** method:
...
To encrypt the data for a [group](/docs/references/glossary#user-groups), obtain a [temporary token](/docs/references/glossary#temp-user-token) using the following **server SDK** method:
...
Depending on the language, multiple formats are supported.  
Besides the _data_, a [metadata](/docs/references/glossary#metadata) can be passed as a second argument. It can be any `JSON serializable object`.
