...
Encryption is completed by a user of your application ([registered](/docs/references/glossary#registered-user) or [unregistered](/docs/references/glossary#anonymous-user)) to another registered user. A user does not need to be logged into blindnet in order to encrypt the data.
...
To encrypt the data for a specific users, obtain a [temporary token](/docs/references/glossary#temp-user-token) using the following **Server SDK** method:
