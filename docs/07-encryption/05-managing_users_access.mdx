...
The core of security of blindnet are user's [private keys](/docs/references/archive/encryption-engine#user-private-keys). They are randomly generated on the user's local device and encrypted using a secret code (e.g. user's password) and then securely stored in blindnet.  
Blindnet allows you to reuse existing user passwords so that your users still use [one password](/docs/references/glossary#secret-value) in your application. To do so safely, you need to use blindnet Client SDK to split user passwords before you connect a user to blindnet (check our [Security page](/docs/references/archive/encryption-engine#user-passwords#user-passwords) to see why this is inportant).
...
When a user logs into your system, you also need to log in that user to blindnet. To do so, you first obtain blindnet secret by [splitting user password](#splitting-secrets), and then use the following **Client SDK** method:
...
The blindnet encryption engine does not expose separate methods for user registration and logging in. You always use the `connect` method while its internals handle user registration and logging in. If it is the first time that a user is being connected to blindnet, the SDK will register that user by using the information you provide in a [token](#creating-user-tokens).

When you use the `connect` method, it locally initiates user's [private keys](/docs/references/archive/encryption-engine#user-private-keys) so that later users can decrypt their data.
...
Since the user's keys are encrypted with the [user's secret](/docs/references/glossary#secret-value), changing the secret means the keys must be re-encrypted with the new secret value.
...
It means Alice will download her [encrypted data keys](/docs/references/glossary#encrypted-data-key) and re-encrypt them with Bob's [public key](/docs/references/glossary#public-key). This way Bob can download and decrypt the keys with his [private key](/docs/references/glossary#private-key).

A possible use case for this is when a new user registers to blindnet and needs access to e.g. customers' data. Think of a new hotel employee accessing guest's encrypted passport during check-in.
...
Revoking access to a user means the user will lose all the keys necessary to decrypt the data and consequently, lose access to data. Your server should inform blindnet about a user for which the access is being revoked, by using the following blindnet **Server SDK** method:
...
In the situation where all users in a [user group](/docs/references/glossary#user-groups) always have access to all the data, this action is not meaningful and [deletion](#deleting-users) of users should be performed instead.
...
When deleting users, you must be careful not to delete the last user of a [group](/docs/references/glossary#user-groups) because in that case all existing data will become undecryptable. Deleting the last user has the same effect as [deleting a user group](#deleting-user-groups).

## Deleting user groups

To delete a [user group](/docs/references/glossary#user-groups), use the following blindnet **Server SDK** method:
