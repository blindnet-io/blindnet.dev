...
## 2. Add Privacy-Rights Management Capabilities to Your Website
...
PRCI is a simple interface allowing your users to assert their privacy rights (therefore covering GDPR and other privacy-related legislation) by sending you a simple yet standardized [Privacy Request](/docs/reference/lexicon#privacy-request).

In order to integrate this interface into your web page, you'll first need to load the associated bundle (included in the `@blindnet/prci` npm package) from the unpkg CDN by adding a `<script>` tag to your HTML document:
...
Here, we use the "core" module build of this package to avoid loading external dependencies like [Lit](https://lit.dev) several times when we'll add other components to this same web page in the following steps.

To better fit any specific context, each Privacy Component for the Web include different builds and bundle, as detailed in the [User Interfaces - Install](/docs/interfaces/install) section.
...
<html lang="en-GB">
  <head>
    <meta charset="utf-8" />
    <title>Privacy Components for the Web - Tutorial</title>
  </head>
  <body>
    <h1>Privacy Components for the Web - Tutorial</h1>
...
Now, open your web app / HTML page in your browser to see this component in action:
...
This includes the classical consultation (access), deletion and modification of their data, of course, but also more advanced ones, like requesting the portability of their data to another system
