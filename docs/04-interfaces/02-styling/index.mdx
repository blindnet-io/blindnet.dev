...
If there is a `<bldn-privacy-portal>` and `<bldn-bridge>` element in an HTML document, the following CSS will give both elements (and all subsequent PC4W elements) a red background:

```css
--bldn-background: red;
```

To have the `<bldn-privacy-portal>` use a different background than all other PC4W elements, use: 

```css
--bldn-background: red;
--bldn-privacy-portal-background: blue;
```