...
Used as a parameter in the [initialization](/docs/encryption/installation#initialization) method of the [Server SDK](#server-side-sdk).

## Application key

A secret value ([private key](#private-key)) which proves the ownership of an [application id](#application-id).
It is used to sign the [tokens](#authentication-token) in the [token generation](/docs/encryption/managing_users_access#creating-user-tokens) methods. The corresponding [public key](#public-key) is stored by blindnet and is used to verify token signatures.  
Used as a parameter in the [initialization](/docs/encryption/installation#initialization) method of the [Server SDK](#server-side-sdk).  
For instructions on how to generate your application keys click [here](/docs/encryption/installation#generating-application-keys).
...
Authentication tokens are used as parameters in the [initialization](/docs/encryption/installation#initialization) methods of the [Client SDK](#client-side-sdk).
...
A token used by the [Server SDK](#server-side-sdk) for various [user and data management](/docs/encryption/managing_users_access) functionalities to authenticate your server.

## Blindnet endpoint

URL of the blindnet api you should use in production. The URL is defined in the [initialization methods](/docs/encryption/installation#initialization) of both [Client](#client-side-sdk) and [Server SDKs](#server-side-sdk).  
...
A set of libraries for front-end applications. Client-side SDK is used for [encryption](/docs/encryption/encrypt) and [decryption](/docs/encryption/decrypt) of data shared between users of your application. Other functionalities are e.g. [giving access](/docs/encryption/managing_users_access#giving-access-to-other-users) to encrypted data to a user who doesn’t possess the [encryption keys](#data-key).

### Server-side SDK

A set of libraries for back-end applications. Server-side SDK is used mainly for user authentication in blindnet by providing the api to [generate the authentication tokens](/docs/encryption/managing_users_access#creating-user-tokens). Additionally, you can use it to [delete users](/docs/encryption/managing_users_access#deleting-users) and [delete encryption keys](/docs/encryption/managing_users_access#deleting-encrypted-data-keys) of your application registered in blindnet.

## Data

Data is anything a user is encrypting. It is in a plain, unencrypted format. Data never leaves user's device until it is [encrypted](/docs/encryption/encrypt).  
Depending on the [Client SDK](#client-side-sdk), multiple formats are supported - `String`, `File`, `byte array`. If the format of the provided data is not supporetd by the SDK, you should encode it to a byte array before [encrypting](/docs/encryption/encrypt), and decode it after [decrypting](/docs/encryption/decrypt).
...
When a [data](#data) is [encrypted](/docs/encryption/encrypt) from any method in the [Client SDK](#client-side-sdk), it can be safely transferred to your or blindnet server.  
Only a user who encrypted the data and the users to whom it was encrypted can decrypt the data.  
To obtain the orginal [data](#data), the encrypted data has to be [decrypted](/docs/encryption/decrypt).

## Encrypted data key

An encrypted [key](#data-key) stored on the blindnet servers.  
Only a user who created the key to [encrypt](/docs/encryption/encrypt) the data and the users to whom it was encrypted have access can decrypt the data key.
...
Additional data that can be [encrypted](/docs/encryption/encrypt) alongside the [data](#data).  
...
If a same [secret value](#secret-value) is used to log-in a user to your application as well as to connect the user to blindnet, the secret must be [hashed and split](/docs/encryption/managing_users_access#splitting-secrets) into two values.  
...
To authenticate a registered user in blindnet, [generate](/docs/encryption/managing_users_access#creating-user-tokens) a [Registered user token](#registered-user-token) which will be sent in every request from the [SDK](#blindnet-sdk) to blindnet.

### Anonymous user

A user which doesn't need to be identified in your application.  
Currently, a (temp user) [token](#temp-user-token) for an anonymous user is used to [encrypt](/docs/encryption/encrypt) the data.

## User groups

A logical grouping of users in your application.  
When [encrypting](/docs/encryption/encrypt) the data, a [group can be specified](/docs/encryption/managing_users_access#assigning-a-user-to-a-group). Any user belonging the specified group will be able to [decrypt](/docs/encryption/decrypt) the data.  
Currently, a user can belong only to a single group which is specified when [registering](/docs/encryption/managing_users_access#connecting-users-with-blindnet) the user to blindnet and it can't be changed afterwards.
