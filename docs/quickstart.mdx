...
### Before you start
Obtain blindnet authentication data. To authenticate to blindnet, your application needs the [application id](./extra/glossary#application-id) and [application key](./extra/glossary#application-key).
...
[Tokens](./extra/glossary#authentication-token) are used to authenticate the users of your application to blindnet. They can be used multiple times and should be regenerated when they expire using the same method. 
...
Use the [application id](./extra/glossary#application-id) and key you obtained in [Before you start](#before-you-start) section.
...
Specify the [testing endpoint](./extra/glossary#testing-endpoint) and [initialize](./guides/installation#initialization) the library for Alice
...
Blindnet can encrypt files with a particular destination (list of users or a [user group](./extra/glossary#user-groups)) in mind. So in order to start encrypting, you first need to have some users (Alice in this case) that you've connected to blindnet.

To connect a user to blindnet, a [secret value](./extra/glossary#secret-value) has to be provided. That value must only be known 
...
We will simulate an [anonymous](./extra/glossary#anonymous-user) user Bob encrypting a file for Alice.

### Initialize SDK for Bob

You need to initialize the library with the [temporary token](./extra/glossary#temp-user-token) in order to encrypt the data. Pass the ids of the users you want to encrypt a file to.
...
### Encrypt a file
Obtain a file from the `<input type="file" id="file-pick>` `HTML` element and pass it to the [encrypt](./guides/encrypt#encrypt) method.
